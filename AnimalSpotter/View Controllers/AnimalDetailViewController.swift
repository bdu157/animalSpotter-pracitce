...
class AnimalDetailViewController: UIViewController {
    ...
    func getDetails() {
        ...
        apiController.fetchDetailsForAnimal(for: animalName) { (result) in
            if let animal = try? result.get() {
                DispatchQueue.main.async {
                    self.updateViews(with: animal)
                }
                //we write fetchImage here because we need to use animal to get url and change it into UIImage.
                self.apiController?.fetchImage(at: animal.imageURL, completion: { (result) in
                    if let image = try? result.get() {
                        DispatchQueue.main.async {
                            self.animalImageView.image = image  //no need to change image data to UIImage since fetch does this for us based on animal.imageURL
                        }
                    }
                })
            }
